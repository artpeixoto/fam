...
impl Path {
    pub fn draw(&self, color: &Color, grid_transform: &GridScreenTransformer) {
        draw_path(&self, color, grid_transform);
    }
}

pub fn draw_path(path: &Path, drawing_info: &Color, grid_screen_transform: &GridScreenTransformer) {
